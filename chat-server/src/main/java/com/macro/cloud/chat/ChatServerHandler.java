...
public class ChatServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ChatServerChannelMap.remove((SocketChannel) ctx.channel());
    }
    ...
    protected void channelRead0(ChannelHandlerContext context, Object obj) throws Exception {
        ...
        if (clientId == null) {
            ...
            System.out.println("服务端收到心跳包，返回响应");
            socketChannel.writeAndFlush(JSON.toJSONString(pingDto));
            return;
        }

        Channel channel = ChatServerChannelMap.get(clientId);

        if (channel==null){
            /**
             * 存放所有连接客户端
             */
            ChatServerChannelMap.add(clientId, socketChannel);
        ...
        System.out.println("我是服务端，收到你的消息了");
